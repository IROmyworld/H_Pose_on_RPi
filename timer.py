...
class Timer():
    ...
    def add_stage(self, name):
        if name not in self.stage.keys():
            self.stage[name] = {'min': np.inf, 'max': 0, 'avg': 0, 'count':0}
    ...
    def update_min_max(self, name, t):
        ...
        if self.update and name in self.stage.keys():
            ...
            new_avg = (t + self.stage[name]['avg'] * (self.stage[name]['count'] \
                       - 1)) / self.stage[name]['count']
            self.stage[name]['avg'] = new_avg
        
        #print(name + ': %7.4f ms, ' % (new_avg), end='')

    def summary(self):
        print('\n\n%15s: %8s %9s %9s %9s' % ('Stage', 'Min', 'Max', 'Avg', 'Count'))
        for name, t in self.stage.items():
            print('%15s' % (name) + ': %8.3f, %8.3f, %8.3f, %8.3s' % (t['min'], t['max'], t['avg'], t['count']))
