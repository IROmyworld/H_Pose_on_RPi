...
class HeadposeEstimation():
    ...
    def __init__(self, lm_type=1, predictor="../models/shape_predictor_68_face_landmarks.dat", verbose=True):
        self.bbox_detector = cv2.CascadeClassifier('../models/haarcascade_frontalface_alt2.xml')        
    ...
    def get_landmarks(self, im):
        ...
        t.tic('bb')
        rectss = self.bbox_detector.detectMultiScale(im, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30), flags=cv2.CASCADE_SCALE_IMAGE)
        if self.v: 
                print(', bb: %.2f' % t.toc('bb'), end='ms')
        for (x, y, w, h) in rectss:
            rects = dlib.rectangle(int(x), int(y), int(x + w), int(y + h))
            

            # Detect landmark of first face
            t.tic('lm')
            landmarks_2d = self.landmark_predictor(im, rects)
            ...
            rect = [rects.left(), rects.top(), rects.right(), rects.bottom()]
    ...
    def process_image(self, im, draw=True, ma=3):
        ...
        if draw:
            ...
            if self.v: 
                print(', draw: %.2f' % t.toc('draw'), end='ms')
         
        return im, angles

if __name__ == '__main__':
    
    RuntimeWarning('This module is not intended to be run as main, please check readme')
